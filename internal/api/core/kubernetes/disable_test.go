...
var _ = Describe("Disable", func() {
	...
	JustBeforeEach(func() {
		kubernetesController.Disable(c, disableManifestRequest)
	})
	...
	When("the manifest name is malformed", func() {
		BeforeEach(func() {
			disableManifestRequest.ManifestName = "malformed"
		})
	...
	Context("#getLoadBalancer", func() {
		When("the load balancer is incorrectly formatted", func() {
			...
			It("returns an error", func() {
				Expect(c.Writer.Status()).To(Equal(http.StatusInternalServerError))
				Expect(c.Errors.Last().Error()).To(Equal("Failed to detach load balancer 'bad-formatting'. Load balancers must be specified in the form '{kind} {name}', e.g. 'service my-service'."))
			})
		})
